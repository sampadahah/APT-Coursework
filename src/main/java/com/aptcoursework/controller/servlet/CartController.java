 ...
 * com.aptcoursework.controller.dao.CartItemDAO; import
 * com.aptcoursework.model.Cart;
 * 
 * public class CartController extends HttpServlet{ private static final long
 * serialVersionUID = 1L;
 * 
 * 
 * public CartController() { super(); } protected void doGet(HttpServletRequest
 * request, HttpServletResponse response) throws ServletException, IOException{
 * 
 * HttpSession session = request.getSession(); Integer userId = (Integer)
 * session.getAttribute("userId");
 * 
 * CartItemDAO cartItemDAO = null; try { cartItemDAO = new CartItemDAO(); }
 * catch (ClassNotFoundException e) { // TODO Auto-generated catch block
 * e.printStackTrace(); } catch (SQLException e) { // TODO Auto-generated catch
 * block e.printStackTrace(); }
 * 
 * List<Cart> cartItems = cartItemDAO.getCartItemByUserID(userId);
 * 
 ...
 * request.getParameter("city"); String streetno =
 * request.getParameter("streetno"); String paymentMethod =
 * request.getParameter("paymentMethod");
 * 
 * String orderSummary = "Order placed sucessfully!\n"; orderSummary +=
 * "Shipping Address:" + address+ "," + city + " " + streetno + "\n";
 * orderSummary += "Payment Method:" + paymentMethod + "\n"; orderSummary +=
 * "Cart Items: \n";
 * 
 * for (Cart item : cartItems) { String name = item.getName(); double price =
 * item.getPrice(); int quantity = item.getQuantity(); orderSummary +=
 * "Product: " + name + ", Price: " + price + ", Quantity: " + quantity + "\n";
 * }
 * 
 * 
 * session.removeAttribute("cartItems"); session.setAttribute("orderMessage",
 * "Your order has been successfully placed!");
 * request.setAttribute("orderSummary", orderSummary);
 * request.getRequestDispatcher("orderConfirmation.jsp").forward(request,
 * response);
 * 
 * } }
 */